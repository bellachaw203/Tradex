...
fn main() {
        ...
        fs::canonicalize(&candidate).unwrap_or_else(|e| {
            panic!(
                "build.rs: cannot find circuits/keys directory at `{}`: {e}\n\
                 ...
                 or set VK_COMMIT_JSON / VK_CANCEL_JSON.",
                candidate.display()
            )
        })
    };
    ...
    // Use commit VK for shared curve params (same Powers of Tau across all VKs)
    writeln!(
        content,
        "const VK_ALPHA_G1: [u8; 64] = {};",
        fmt_bytes(&g1_bytes(&commit_vk["vk_alpha_1"]))
    )
    .unwrap();
    writeln!(
        content,
        "const VK_BETA_G2: [u8; 128] = {};",
        fmt_bytes(&g2_bytes(&commit_vk["vk_beta_2"]))
    )
    .unwrap();
    writeln!(
        content,
        "const VK_GAMMA_G2: [u8; 128] = {};",
        fmt_bytes(&g2_bytes(&commit_vk["vk_gamma_2"]))
    )
    .unwrap();
    writeln!(
        content,
        "const VK_DELTA_G2: [u8; 128] = {};",
        fmt_bytes(&g2_bytes(&commit_vk["vk_delta_2"]))
    )
    .unwrap();
