...
const chartTheme = {
  gruvbox: {
    ...
    grid: 'rgba(60,56,54,0.10)',
    gridOff: 'rgba(60,56,54,0)',
    text: 'rgba(60,56,54,0.72)',
  ...
  nord: {
    panel: '#eceff4',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(46,52,64,0.10)',
    gridOff: 'rgba(46,52,64,0)',
    text: 'rgba(46,52,64,0.72)',
  ...
  solarized: {
    ...
    gridOff: 'rgba(7,54,66,0)',
    text: 'rgba(7,54,66,0.72)',
    border: 'rgba(7,54,66,0.14)',
  },
  tokyo: {
    panel: '#1a1b26',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(192,202,245,0.07)',
    gridOff: 'rgba(192,202,245,0)',
    text: 'rgba(192,202,245,0.72)',
    border: 'rgba(192,202,245,0.10)',
  },
  dracula: {
    panel: '#282a36',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(248,248,242,0.07)',
    gridOff: 'rgba(248,248,242,0)',
    text: 'rgba(248,248,242,0.72)',
    border: 'rgba(248,248,242,0.10)',
  },
  matrix: {
    panel: '#07110d',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(105,240,174,0.08)',
    gridOff: 'rgba(105,240,174,0)',
    text: 'rgba(216,255,231,0.72)',
    border: 'rgba(105,240,174,0.12)',
  },
  sepia: {
    panel: '#fff4dc',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(47,38,31,0.10)',
    gridOff: 'rgba(47,38,31,0)',
    text: 'rgba(47,38,31,0.72)',
    border: 'rgba(47,38,31,0.13)',
  },
  slate: {
    panel: '#171d23',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(240,244,248,0.07)',
    gridOff: 'rgba(240,244,248,0)',
    text: 'rgba(240,244,248,0.72)',
    border: 'rgba(240,244,248,0.09)',
  },
  contrast: {
    panel: '#ffffff',
    toolbar: 'bg-surface-primary',
    grid: 'rgba(0,0,0,0.13)',
    gridOff: 'rgba(0,0,0,0)',
...
function volColor(c: Candle): string {
  ...
  return c.close >= c.open ? `rgba(52,211,153,${alpha})` : `rgba(200,70,30,${alpha})`
}
...
function bucketCandles(points: MarketCandle[], bucket: number): Candle[] {
  ...
  for (const p of points) {
    ...
    if (!c) {
      ...
      c.high = Math.max(c.high, p.high)
      c.low = Math.min(c.low, p.low)
      c.close = p.close
      c.volume += p.volume
    }
  }
...
class VolProfilePrimitive {
  ...
  update(d: VPData | null) {
    this._d = d
    this._api?.requestUpdate()
  }
  paneViews() {
    return this._d ? [new VPPaneView(this._d)] : []
  }
}
// ─────────────────────────────────────────────────────────────────────────────
...
function vpStep(range: number, n = 38) {
  const raw = range / n,
    exp = Math.floor(Math.log10(Math.max(raw, 1e-10))),
    b = Math.pow(10, exp)
...
export default function PriceChart() {
  ...
  function updateVP() {
    ...
    for (let i = fromIdx; i <= toIdx; i++) {
      ...
      if (c.close >= c.open) b.buy += c.volume
      else b.sell += c.volume
      bkts.set(bi, b)
    }
    if (!bkts.size) return
    ...
    if (cs) {
      if (pocLineRef.current) {
        cs.removePriceLine(pocLineRef.current)
        pocLineRef.current = null
      }
      if (vahLineRef.current) {
        cs.removePriceLine(vahLineRef.current)
        vahLineRef.current = null
      }
      if (valLineRef.current) {
        cs.removePriceLine(valLineRef.current)
        valLineRef.current = null
      }
      if (pocKey >= 0)
        pocLineRef.current = cs.createPriceLine({
          price: pocKey * tick + tick / 2,
          color: 'rgba(200,220,50,0.70)',
          lineWidth: 1,
          lineStyle: LineStyle.Dashed,
          axisLabelVisible: true,
          title: 'POC',
          axisLabelColor: 'rgba(200,220,50,0.85)',
          axisLabelTextColor: '#000',
        })
      if (vahKey >= 0)
        vahLineRef.current = cs.createPriceLine({
          price: vahKey * tick + tick,
          color: 'rgba(52,211,153,0.45)',
          lineWidth: 1,
          lineStyle: LineStyle.Dotted,
          axisLabelVisible: true,
          title: 'VAH',
          axisLabelColor: 'rgba(52,211,153,0.6)',
          axisLabelTextColor: '#000',
        })
      if (valKey >= 0)
        valLineRef.current = cs.createPriceLine({
          price: valKey * tick,
          color: 'rgba(200,70,30,0.45)',
          lineWidth: 1,
          lineStyle: LineStyle.Dotted,
          axisLabelVisible: true,
          title: 'VAL',
          axisLabelColor: 'rgba(200,70,30,0.6)',
          axisLabelTextColor: '#000',
        })
    }
  }
  ...
  useEffect(() => {
    ...
    const chart = createChart(wrapRef.current, {
      ...
      crosshair: {
        ...
        vertLine: { color: CROSS, width: 1, style: LineStyle.Dashed, labelBackgroundColor: ENTRY },
        horzLine: { color: CROSS, width: 1, style: LineStyle.Dashed, labelBackgroundColor: ENTRY },
      },
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      if (ttDelta.current) {
        ttDelta.current.textContent = `${isUp ? '+' : ''}${delta.toFixed(2)} (${isUp ? '+' : ''}${pct.toFixed(2)}%)`
        ttDelta.current.style.color = col
      }
  ...
  useEffect(() => {
    ...
    const volData = showVolume
      ? data.map((c) => ({ time: c.time, value: c.volume, color: volColor(c) }))
      : []
    ...
    if (vwapRef.current) {
      ...
      vwapRef.current.setData(w.vwap)
      vwapU1Ref.current?.setData(w.u1)
      vwapL1Ref.current?.setData(w.l1)
      vwapU2Ref.current?.setData(w.u2)
      vwapL2Ref.current?.setData(w.l2)
    }
  ...
  useEffect(() => {
    ...
    if (showBB) {
      ...
      if (!chart2) return
      if (bbUpperRef.current) {
        chart2.removeSeries(bbUpperRef.current)
        bbUpperRef.current = null
      }
      if (bbMidRef.current) {
        chart2.removeSeries(bbMidRef.current)
        bbMidRef.current = null
      }
      if (bbLowRef.current) {
        chart2.removeSeries(bbLowRef.current)
  ...
  useEffect(() => {
    candleRef.current?.applyOptions({
      upColor: hollowCandles ? UP_FILL : UP,
      downColor: DOWN,
      borderUpColor: UP,
      borderDownColor: DOWN,
      wickUpColor: UP,
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface-primary">
      {/* ── Toolbar ── */}
      <div
        className={`flex min-h-[78px] shrink-0 flex-col border-b border-border-subtle ${colors.toolbar}`}
      >
        <div className="flex h-10 items-center gap-2 px-3">
          ...
          <Btn active title="Candles">
            <IconChartCandle size={14} stroke={1.8} />
          </Btn>
          <Btn
            active={hollowCandles}
            onClick={() => setHollowCandles((v) => !v)}
            title="Hollow candles"
          >
            <IconRectangle size={14} stroke={1.8} />
            <span>Hollow</span>
          </Btn>
          <Btn
            active={showMA}
            onClick={() => setShowMA((v) => !v)}
            title="Moving averages (9 / 21)"
          >
          ...
          <Btn
            ...
            title="Footprint — bid/ask volume per tick"
          >
          ...
          <div className="ml-auto hidden items-center gap-3 text-[11px] text-text-tertiary xl:flex">
            <Stat
              label="Index"
              value={index.toLocaleString('en-US', { maximumFractionDigits: 2 })}
            />
        ...
        <div className="flex min-w-0 items-baseline gap-2 px-4 pb-2 text-[12px]">
          ...
          {last && (
            <>
              ...
              {showMA && (
                <span className="ml-1 flex gap-2 text-[11px]">
                  <span style={{ color: MA_FAST }}>MA9</span>
                  <span style={{ color: MA_SLOW }}>MA21</span>
                </span>
              )}
              {showBB && (
                <span className="text-[11px]" style={{ color: BB_COL }}>
                  BB(20)
                </span>
              )}
              {showRSI && (
                <span className="text-[11px]" style={{ color: RSI_COL }}>
                  RSI(14)
                </span>
              )}
            </>
      ...
      <div className="relative min-h-0 flex-1">
        {showFP ? (
          <Suspense
            fallback={<div className="h-full w-full" style={{ background: colors.panel }} />}
          >
            <FootprintChart />
          </Suspense>
        ) : (
        ...
        >
          ...
          <div className="grid grid-cols-[14px_1fr] gap-x-2 gap-y-0.5 text-[10px]">
            <span className="text-text-quaternary">O</span>
            <span ref={ttO} className="tabular-nums text-text-secondary" />
            <span style={{ color: UP }}>H</span>{' '}
            <span ref={ttH} className="tabular-nums" style={{ color: UP }} />
            <span style={{ color: DOWN }}>L</span>{' '}
            <span ref={ttL} className="tabular-nums" style={{ color: DOWN }} />
            <span className="text-text-quaternary">C</span>
            <span ref={ttC} className="tabular-nums text-text-secondary" />
            <span className="text-text-quaternary">V</span>
            <span ref={ttV} className="tabular-nums text-text-quaternary" />
          </div>
        </div>
      </div>
    </div>
