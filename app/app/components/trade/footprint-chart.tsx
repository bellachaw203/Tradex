...
interface FPBar extends Candle {
  levels: Level[]
}
...
const GRID = {
  gruvbox: 'rgba(60,56,54,0.09)',
  light: 'rgba(17,24,39,0.07)',
  dark: 'rgba(255,255,255,0.04)',
  nord: 'rgba(46,52,64,0.09)',
  solarized: 'rgba(7,54,66,0.09)',
  tokyo: 'rgba(192,202,245,0.06)',
  dracula: 'rgba(248,248,242,0.06)',
  matrix: 'rgba(105,240,174,0.07)',
  sepia: 'rgba(47,38,31,0.09)',
  slate: 'rgba(240,244,248,0.06)',
  contrast: 'rgba(0,0,0,0.12)',
} as const
const TXT = {
  gruvbox: 'rgba(60,56,54,0.72)',
  light: 'rgba(17,24,39,0.68)',
  dark: 'rgba(255,255,255,0.68)',
  nord: 'rgba(46,52,64,0.72)',
  solarized: 'rgba(7,54,66,0.72)',
  tokyo: 'rgba(192,202,245,0.70)',
  dracula: 'rgba(248,248,242,0.70)',
  matrix: 'rgba(216,255,231,0.70)',
  sepia: 'rgba(47,38,31,0.72)',
  slate: 'rgba(240,244,248,0.70)',
  contrast: 'rgba(0,0,0,0.82)',
} as const
...
function fmtP(p: number, step: number) {
  return p.toFixed(Math.max(0, -Math.floor(Math.log10(step))))
}
...
function buildFP(c: Candle, tick: number): Level[] {
  ...
  for (let p = lo; p <= hi + tick * 0.01; p = +(p + tick).toFixed(10)) {
    const w =
      Math.exp(-((Math.abs(p + tick / 2 - vwap) / range) ** 2) * 4) *
      (rand(seed + Math.round(p / tick)) * 0.35 + 0.65)
    ws.push(w)
    tw += w
  }
...
export default function FootprintChart() {
  ...
  function paint() {
    ...
    const rightIdx = Math.max(
      0,
      Math.min(data.length - 1, data.length - 1 - Math.round(Math.max(0, scroll)))
    )
    ...
    const tick =
      sampleLevels && sampleLevels.length > 1
    ...
    let maxBkt = 1
    for (const b of vpvrBuckets.values()) {
      const t2 = b.buy + b.sell
      if (t2 > maxBkt) maxBkt = t2
    }
    let pocKey = -1,
      pocVol = 0
    ...
    ctx.fillStyle = bg
    ctx.fillRect(vx, 0, VPVR_W, chartH)
    ...
    ctx.stroke()
    ctx.beginPath()
    ctx.moveTo(vx + VPVR_W, 0)
    ctx.lineTo(vx + VPVR_W, chartH)
    ctx.stroke()
    ...
    for (let i = leftIdx; i <= rightIdx; i++) {
      ...
      ctx.beginPath()
      ctx.rect(
        Math.max(PAD_L, x - halfW),
        0,
        Math.min(bw, chartW - Math.max(PAD_L, x - halfW)),
        chartH
      )
      ctx.clip()
      ...
      for (const lv of bar.levels) {
        ...
        const alpha = whale ? 0.8 : Math.min(0.55, 0.08 + (tot / maxVol) * 1.8)
        const gap = 1

        // Bid cell
        ctx.fillStyle = whale
          ? `rgba(200,220,50,${alpha.toFixed(2)})`
          : `rgba(52,211,153,${alpha.toFixed(2)})`
        ctx.fillRect(x - halfW + gap, yTop + gap * 0.5, halfW - gap * 1.5, cellH - gap)
        // Ask cell
        ctx.fillStyle = whale
          ? `rgba(220,160,30,${alpha.toFixed(2)})`
          : `rgba(200,70,30,${alpha.toFixed(2)})`
      ...
      if (bar.close >= bar.open) {
        ctx.strokeStyle = UP
        ctx.lineWidth = Math.max(1.5, barW * 0.022)
        ctx.strokeRect(
          Math.round(x - bW / 2) + 0.5,
          Math.round(bTop) + 0.5,
          Math.round(bW),
          Math.round(bH)
        )
    ...
    if (my >= 0 && my <= chartH) {
      ...
      ctx.fillRect(chartW + VPVR_W + 1, my - 9, lw, 18)
      ctx.fillStyle = '#fff'
      ctx.textAlign = 'left'
      ctx.textBaseline = 'middle'
    ...
    if (cx >= 0 && cx < chartW && cy >= 0 && cy < chartH) {
      ...
      ctx.fillRect(chartW + VPVR_W + 1, cy - 8, clw, 16)
      ctx.fillStyle = '#fff'
      ctx.textAlign = 'left'
      ctx.textBaseline = 'middle'
    ...
    ctx.fillStyle = bg
    ctx.fillRect(axX, 0, AXIS_W, H)
    ctx.strokeStyle = grd
    ctx.lineWidth = 1
    ctx.beginPath()
    ctx.moveTo(axX + 0.5, 0)
    ctx.lineTo(axX + 0.5, H)
    ctx.stroke()
    ctx.font = '10px "Berkeley Mono", monospace'
    ctx.fillStyle = txt
    ctx.textAlign = 'left'
  ...
  const onDown = (e: React.MouseEvent) => {
    ...
    st.current.dragX = e.clientX
    st.current.dragS = st.current.scroll
  }
  const onMove = (e: React.MouseEvent) => {
    const r = canvasRef.current!.getBoundingClientRect()
    st.current.cx = e.clientX - r.left
    st.current.cy = e.clientY - r.top
    if (st.current.drag)
      st.current.scroll = Math.max(
        0,
        st.current.dragS - (e.clientX - st.current.dragX) / st.current.barW
      )
    schedule()
  }
  const onLeave = () => {
