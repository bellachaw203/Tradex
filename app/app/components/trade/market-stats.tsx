...
export default function MarketStats() {
  ...
  return (
    <div className="grid h-full grid-cols-2 gap-px bg-border-subtle">
      ...
      <Tile
        label="Funding / 8h"
        value={`${funding >= 0 ? '+' : ''}${(funding * 100).toFixed(4)}%`}
        accent
      />
      <Tile label="Next funding" value="—" />
      <Tile
        label="Open interest"
        value={openInterest != null ? formatCompactUsd(openInterest) : '—'}
      />
...
function Tile({
  ...
  return (
    <div className="flex min-w-0 flex-col justify-center gap-1 bg-surface-primary px-3 py-2">
      ...
      <span
        className={`truncate text-[14px] font-semibold tabular-nums ${accent ? 'text-brand-violet' : 'text-text-secondary'}`}
      >
        {value}
      </span>
    </div>
