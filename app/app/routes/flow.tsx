...
const NODES: NodeDef[] = [
  {
    ...
    bullets: [
      'Encrypt order → TEE pubkey',
      'Sign Stellar txns',
      'Store note secrets locally',
  ...
  },
  {
    ...
    bullets: [
      ...
      'Groth16 prover · arkworks BN254',
      'Poseidon2 commitment hash',
      'KMS-backed signing key',
      'WAL persistence · RwLock',
    ],
  },
  {
    id: 'stellar',
    ...
    bullets: [
      'verify_groth16() via BN254 host fns',
      'perp-engine: positions, collateral',
      'orderbook: commitments, nullifiers',
      'collateral-token: USDC mint/burn',
      'Oracle prices (set_asset_price)',
    ],
  },
  {
    id: 'pyth',
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <span style={{ fontSize: 13, color: C.dim, fontFamily: 'ui-monospace, monospace' }}>
          System Flow
        </span>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '0 24px 32px' }}>
        <div style={{ maxWidth: 1340, margin: '0 auto' }}>
          ...
          >
            ...
            {/* Node boxes */}
            {NODES.map((n) => (
              <NodeBox key={n.id} node={n} active={active === n.id} onClick={() => toggle(n.id)} />
            ))}

            {/* Step numbers on edge midpoints */}
            {EDGES.filter((e) => e.step !== undefined).map((e) => (
              <g key={`step-${e.id}`}>
                <circle cx={e.labelX - 14} cy={e.labelY - 5} r="7" fill={e.color} opacity="0.18" />
              </g>
            ))}
          </svg>
        </div>
