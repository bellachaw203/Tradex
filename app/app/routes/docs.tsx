...
export const meta = () => [
  ...
  {
    name: 'description',
    content: 'How Tradex works: ZK circuits, TEE architecture, and live markets.',
  },
]
...
function Pill({ children, color }: { children: string; color?: string }) {
  return (
    <span
      ...
      style={{
        background: color ?? 'rgba(128,125,254,0.12)',
        color: color ? '#fff' : 'var(--color-brand-violet)',
      }}
    >
      {children}
    </span>
  ...
  return (
    <div className="min-h-screen bg-page text-text-primary">
      ...
      <header className="sticky top-0 z-50 flex items-center gap-4 border-b border-border-subtle bg-surface-primary/80 px-6 py-3 backdrop-blur-sm">
        <Link
          to="/"
          className="flex items-center gap-2.5 text-[13px] font-semibold text-text-primary hover:opacity-80"
        >
      ...
      <div className="mx-auto flex max-w-[1100px] gap-10 px-6 py-12">
        ...
        <aside className="hidden w-52 shrink-0 lg:block">
          <div className="sticky top-24 space-y-0.5">
            <p className="mb-3 text-[10px] uppercase tracking-widest text-text-quaternary">
              On this page
            </p>
            {TOC.map((item) => (
              <a
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="mb-12 flex items-start gap-5">
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {['Groth16 / BN254', 'GCP Confidential Space', 'Stellar Soroban', 'Rust'].map(
                  (t) => (
                    <Pill key={t}>{t}</Pill>
                  )
                )}
          ...
          <section id="what">
            ...
            <P>
              ...
              <a
                ...
                className="text-brand-violet underline-offset-2 hover:underline"
              >
                Stellar Soroban
              </a>
              . It combines three layers of technology to give traders institutional-grade privacy:
              no position is ever stored in plaintext, no collateral amount is ever visible, and the
              chain only sees commitment hashes, nullifiers, and proof verification results.
            </P>
            ...
            <Table
              ...
              rows={[
                [
                  ...
                  'Proves order validity, matching, and collateral spend on-chain — without revealing private inputs',
                ],
                [
                  'TEE',
                  'GCP Confidential Space (AMD SEV-SNP)',
                  'Runs the matching engine inside an encrypted enclave. Inputs are encrypted to the TEE; the host operator sees nothing',
                ],
                [
                  'Settlement',
                  'Stellar Soroban (Protocol 26)',
                  'Verifies Groth16 proofs via BN254 host functions. Stores commitments, nullifiers, and positions',
                ],
              ]}
            />
          </section>
          ...
          <section id="how">
            ...
            <P>
              A user deposits USDC into the perp engine. The deposit creates a{' '}
              <strong>shielded note</strong>: a Poseidon2 commitment to{' '}
              <Code>(amount, secret)</Code> stored on-chain. Only the depositor knows the preimage.
              Funds in the shielded pool cannot be linked to any position or withdrawal without the
              secret.
            </P>
            ...
            <P>
              The client sends encrypted order parameters to the TEE. The TEE generates a Groth16
              proof inside the enclave — inputs never leave the SEV-SNP boundary — and returns a
              commitment built from a Poseidon2 hash chain over all order fields:
            </P>
...
commitment = Poseidon2(h6, secret, domain=7)`}</Pre>
            <P>
              Stellar verifies the Groth16 proof via BN254 MSM and pairing host functions, then
              registers the order in the CLOB.
            </P>

            <H3>3. Match — OrderMatch proof</H3>
            <P>
              When two orders cross in the TEE's CLOB engine, the enclave generates an OrderMatch
              proof that proves in zero-knowledge:
            </P>
            <ul className="mb-6 space-y-1.5 pl-5 text-[14px] text-text-secondary">
              {[
                ...
                <>
                  ...
                  <Code>Poseidon2(commitment, match_price, match_size, domain=10)</Code>
                </>,
              ].map((item, i) => (
            ...
            <P>
              Public outputs:{' '}
              <Code>(cmt_a, cmt_b, match_price, match_size, nullifier_a, nullifier_b)</Code> — no
              private order details exposed.
            </P>

            <H3>4. Open Position — note spend</H3>
            <P>
              Opening a matched position requires two proofs in sequence: a{' '}
              <strong>NoteSpend</strong> proof (proving the trader knows the secret for a shielded
              note with enough collateral), and an <strong>OrderCommitment</strong> proof binding
              the position to the matched order. The note nullifier is published, collateral locked,
              and the position commitment stored on-chain.
            </P>

            <H3>5. Close / Withdraw</H3>
            <P>
              Closing a position or withdrawing from the shielded pool requires a NoteSpend proof.
              The nullifier is checked for uniqueness (replay protection), collateral returned, and
              the nullifier marked spent permanently.
            </P>
          </section>
          ...
          <section id="trading">
            ...
            <Table
              ...
              rows={[
                [
                  'Market',
                  'Fills immediately at best available price. No price constraint in the ZK proof.',
                ],
                ['Limit', 'Rests in the book until the market price crosses the declared limit.'],
                [
                  'Stop-Limit',
                  'Dormant until mark price crosses the stop trigger, then activates as a limit order.',
                ],
                [
                  'Stop-Market',
                  'Dormant until mark price crosses the stop trigger, then activates as a market order.',
                ],
                [
                  'IOC (Immediate-or-Cancel)',
                  'Fills whatever is available now, cancels the rest. Never rests in the book.',
                ],
                ['FOK (Fill-or-Kill)', 'Fills the entire size immediately or rejects entirely.'],
              ]}
            />
            <P>
              Stop orders sit in a separate stop book inside the TEE. The engine scans and triggers
              them on every mark price update:
            </P>
            <ul className="mb-6 space-y-1.5 pl-5 text-[14px] text-text-secondary">
              {[
                <>
                  Bid stops trigger when <Code>mark_price ≥ stop_price</Code> — stop-loss on shorts,
                  buy-stop breakouts
                </>,
                <>
                  Ask stops trigger when <Code>mark_price ≤ stop_price</Code> — stop-loss on longs,
                  sell-stop breakouts
                </>,
              ].map((item, i) => (
                <li key={i} className="flex items-start gap-2">
            ...
            <P>
              TP and SL are implemented as a pair of stop orders placed alongside the opening
              commitment:
            </P>
            <ul className="mb-6 space-y-1.5 pl-5 text-[14px] text-text-secondary">
              {[
                <>
                  <strong className="text-text-primary">Take Profit</strong> — Stop-Limit on the
                  opposite side at your target price
                </>,
                <>
                  <strong className="text-text-primary">Stop Loss</strong> — Stop-Market on the
                  opposite side at your risk limit
                </>,
              ].map((item, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-brand-violet" />
            ...
            <P>
              Both reference the same <Code>position_commitment</Code>. When either triggers and
              fills, the matching proof nullifies the position commitment so the other is
              automatically invalidated on settlement.
            </P>

            <H3>Leverage</H3>
            <P>
              Up to <strong>50× leverage</strong> on crypto markets,{' '}
              <strong>10–20× on RWA markets</strong>, enforced on-chain — the contract validates{' '}
              <Code>leverage ≤ asset.max_leverage</Code> and rejects if exceeded.
            </P>
...
liq_price (short) = entry × (1 + 0.92 / leverage)`}</Pre>
            ...
            <P>
              Configured via a single boolean witness <Code>use_cross</Code> in the{' '}
              <Code>OrderCommitment</Code> circuit — proved in zero-knowledge, so margin mode is
              verifiable without revealing it.
            </P>
            <Table
              head={['Mode', 'Behaviour']}
              rows={[
                [
                  'Isolated',
                  'Each position has its own collateral bucket. A loss cannot spill to other positions.',
                ],
                [
                  'Cross',
                  'Positions share a portfolio collateral pool, linked by portfolio_key = Poseidon2(secret, 0, domain=20).',
                ],
              ]}
            />
            <P>
              The on-chain state only ever sees the <Code>portfolio_key</Code> hash — not the secret
              or the trader's identity.
            </P>

            <H3>Liquidation</H3>
            <P>
              Any address can call <Code>liquidate(position_commitment)</Code>. The contract:
            </P>
            <ul className="mb-6 space-y-1.5 pl-5 text-[14px] text-text-secondary">
              {[
                <>
                  Reads the position's <Code>collateral</Code>, <Code>leverage</Code>,{' '}
                  <Code>entry_price</Code>, and current <Code>mark_price</Code> from oracle
                </>,
                <>
                  Computes unrealised PnL:{' '}
                  <Code>pnl = (mark − entry) / entry × collateral × leverage × direction</Code>
                </>,
                <>
                  If <Code>collateral + pnl {'<'} maintenance_margin</Code>, the position is
          ...
          <section id="circuits">
            ...
            <P>
              ...
              — BN254 + Groth16. No Circom, no NoirLang. Everything is native R1CS over the BN254
              scalar field, hand-written with <Code>ark-r1cs-std</Code>. The hash primitive is{' '}
              <strong>Poseidon2</strong> (width-3 sponge, distinct domain separators per round),
              natively supported by Stellar Soroban's Protocol 26 BN254 host functions.
            </P>
            ...
            <Table
              ...
              rows={[
                [
                  'OrderCommitment',
                  'commitment, portfolio_key',
                  'Poseidon2 chain over all 8 order fields = commitment. Optionally proves cross-margin group key.',
                ],
                [
                  'NoteSpend',
                  'note_commitment, nullifier',
                  'note_commitment = Poseidon2(amount, secret, 8) and nullifier = Poseidon2(note_commitment, secret, 9)',
                ],
                [
                  'OrderMatch',
                  'cmt_a, cmt_b, match_price, match_size, nullifier_a, nullifier_b',
                  'Both commitments valid, opposite sides, same asset, price constraints, size bounds, correct nullifiers',
                ],
                [
                  'OrderCancel',
                  'commitment, cancel_nullifier',
                  'Holder knows the secret behind the commitment and derives a valid cancel nullifier',
                ],
                [
                  'ShieldedInsert',
                  'root_before, root_after, leaf',
                  'Merkle tree insertion was performed correctly',
                ],
                [
                  'ShieldedWithdraw',
                  'root, nullifier',
                  'Leaf exists in tree and nullifier correctly derived from it',
                ],
              ]}
            />
            ...
            <P>
              For limit orders the circuit enforces fill-price correctness via conditional range
              checks using bit-decomposition over the 254-bit BN254 field. For a bid (
              <Code>side=0</Code>):
            </P>
            <Pre>{`match_price ≤ declared_limit_price    // buyer filled at or better`}</Pre>
            <P>
              For an ask (<Code>side=1</Code>):
            </P>
            <Pre>{`declared_limit_price ≤ match_price    // seller filled at or better`}</Pre>
            <P>
              The <Code>enforce_cond_le</Code> gadget gates these checks with <Code>is_market</Code>{' '}
              — market orders bypass the price constraint entirely.
            </P>

            <H3>Cross-margin extension</H3>
            <P>
              <Code>OrderCommitment</Code> supports isolated and cross-margin via a single boolean
              witness <Code>use_cross</Code>. When set, it additionally proves:
            </P>
            <Pre>{`portfolio_key = use_cross × Poseidon2(secret, 0, domain=20)`}</Pre>
            <P>
              A zero <Code>portfolio_key</Code> means isolated margin. Non-zero groups positions
              into a cross-margin portfolio. The secret is never revealed.
            </P>
          </section>
          ...
          <section id="tee">
            ...
            <P>
              ...
              <a
                href="https://cloud.google.com/confidential-computing"
                target="_blank"
                rel="noopener noreferrer"
                className="text-brand-violet underline-offset-2 hover:underline"
              >
              ...
              with AMD SEV-SNP attestation. The enclave holds the Groth16 proving keys for all
              circuits. It decrypts order inputs, runs the CLOB, and generates proofs — all inside
...
└─────────────────────────────────────────────────────────────┘`}</Pre>
          ...
          <section id="markets">
            ...
            <P>
              ...
              real-time WebSocket feeds. Charts use Pyth Benchmarks for historical OHLCV and Hermes
              WebSocket for live 1-minute candle updates. The chart shows the{' '}
              <strong>index price</strong> — the oracle feed — which is standard for oracle-priced
              perp venues (GMX, dYdX, Hyperliquid all do this).
            </P>
            <Table
          ...
          <section id="keepers">
            ...
            <H3>Oracle keeper</H3>
            <P>
              Fetches Pyth prices every 30 seconds, submits <Code>set_asset_price</Code> on-chain
              for each market via <Code>stellar</Code> CLI + Soroban RPC.
            </P>

            <H3>Market maker</H3>
            <P>
              32 bid levels + 32 ask levels per market. Size grows geometrically:{' '}
              <Code>base_size × 1.08^level</Code>. Spread formula per category:
            </P>
            <Pre>{`Crypto markets:  (5 + 3×level) bps
RWA markets:    (10 + 5×level) bps`}</Pre>
            <P>
              Re-quotes when mid moves {'>'} 0.5% or quotes are stale ({'>'} 5 min TTL). Commitment
              proofs are pre-generated in a pool to avoid proof latency during re-balancing.
            </P>

            <H3>Liquidator</H3>
            <P>
              Scans a watchlist of matched positions and calls <Code>liquidate()</Code> on any
              commitment that falls below the maintenance margin threshold.
            </P>
          </section>
